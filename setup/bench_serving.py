                ...
                if response.status == 200:
                    async for chunk_bytes in response.content:
                        ...
                        if chunk == "[DONE]":
                            ...
                            if "choices" in data and data.get("choices") and data["choices"][0].get("text"):
                                timestamp = time.perf_counter()
                                # First token
                                if ttft == 0.0:
                                    ttft = time.perf_counter() - st
...
class BenchmarkMetrics:
    ...
    p75_itl_ms: float
    p99_itl_ms: float
    p25_e2e_latency_ms: float
    p75_e2e_latency_ms: float
...
def calculate_metrics(
    ...
    metrics = BenchmarkMetrics(
        ...
        total_token_throughput=(total_input+sum(output_lens))/dur_s,
        ...
        * 1000,  # ttfts is empty if streaming is not supported by backend
        ...
        p25_e2e_latency_ms=np.percentile(e2e_latencies or 0, 25) * 1000,
        p75_e2e_latency_ms=np.percentile(e2e_latencies or 0, 75) *1000,
...
async def benchmark(
    ...
    print("TTFTs:", json.dumps(metrics.ttfts))
    print("ITLs:", json.dumps(metrics.itls))
    print("TPOTs:", json.dumps(metrics.tpots))
    print("E2E Latencies:", json.dumps(metrics.e2es))
    ...
    print("{:<40} {:<10.2f}".format("P75 E2E Latency (ms):", metrics.p75_e2e_latency_ms))
    print("{:<40} {:<10.2f}".format("P25 E2E Latency (ms):", metrics.p25_e2e_latency_ms))
    ...
    result = {
        ...
        "p25_ttft_ms":metrics.p25_ttft_ms,
        "p75_ttft_ms":metrics.p75_ttft_ms,
        "mean_tpot_ms": metrics.mean_tpot_ms,
        "median_tpot_ms": metrics.median_tpot_ms,
        "std_tpot_ms": metrics.std_tpot_ms,
        "p99_tpot_ms": metrics.p99_tpot_ms,
        "p75_tpot_ms": metrics.p75_tpot_ms,
        "p25_tpot_ms": metrics.p25_tpot_ms,
        "mean_itl_ms": metrics.mean_itl_ms,
        "median_itl_ms": metrics.median_itl_ms,
        "std_itl_ms": metrics.std_itl_ms,
        "p99_itl_ms": metrics.p99_itl_ms,
        "p75_itl_ms": metrics.p75_itl_ms,
        "p25_itl_ms": metrics.p25_itl_ms,
        ...
        "p25_e2e_latency_ms": metrics.p25_e2e_latency_ms,
        "p75_e2e_latency_ms": metrics.p75_e2e_latency_ms,
