...
OUTPUT_DIR="sglang_offline_nps4_${MODE}_5000_$(date +'%Y-%m-%d')"

# Create the output directory
mkdir -p "$OUTPUT_DIR"

# Output CSV file
CSV_FILE="${OUTPUT_DIR}/sglang_offline_discuda_nps4_${MODE}_$(date +'%Y-%m-%d').csv"
RATIO_CSV_FILE="${OUTPUT_DIR}/sglang_offline_ratio_nps4_${MODE}_$(date +'%Y-%m-%d').csv"
ERROR_LOG="${OUTPUT_DIR}/sglang_offline_errors_nps4_${MODE}_$(date +'%Y-%m-%d').log"
MASTER_LOG="${OUTPUT_DIR}/sglang_master_offline_nps4_${MODE}_$(date +'%Y-%m-%d').log"
...
#declare -a MAX_NUM_SEQUENCES=(1 2 4 8 16 32 64 128 256 512 1024 2048)
declare -a MAX_NUM_SEQUENCES=(32 16 8 4)
...
run_benchmark() {
    ...
    setsid python -m sglang.launch_server --model-path "$model_path" --max-running-requests "$max_num_seqs" --max-prefill-tokens 8192 --disable-cuda-graph > server.log 2>&1 &
    ...
    sleep 5
    local result
    if ! python3 -m sglang.bench_serving --backend sglang --dataset-name random --random-output-len 128 --random-input-len 128 --num-prompts 5000 --random-range-ratio 1  > benchmark.log 2>&1; then
