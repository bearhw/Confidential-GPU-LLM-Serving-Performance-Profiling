...
OUTPUT_DIR="vllm_offline_nps4_v1_compile0_graph0_${MODE}_5000_$(date +'%Y-%m-%d')"
...
run_benchmark() {
  ...
  COMPILATION_JSON='{"level": 0, "use_cudagraph":false}'
  setsid env VLLM_DISABLE_COMPILE_CACHE=1 VLLM_TORCH_COMPILE_LEVEL=0 python -m vllm.entrypoints.openai.api_server \
    --model "$model_path" \
    --disable-log-requests \
    --max_model_len 4096 \
    --max-num-seqs "$max_num_seqs" \
    --enforce-eager "True" \
    --compilation-config "$COMPILATION_JSON" \
