...
OUTPUT_DIR="sglang_online_withoutChunk_largeInput_nps4_${MODE}_5000_$(date +'%Y-%m-%d')"
...
run_benchmark() {
    ...
    setsid python -m sglang.launch_server --model-path "$model_path" --max-running-requests 128  --max-prefill-tokens 8192 > server.log 2>&1 &
