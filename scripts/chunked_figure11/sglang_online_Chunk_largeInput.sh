...
OUTPUT_DIR="sglang_online_Chunk_nps4_largeInput8K_${MODE}_5000_$(date +'%Y-%m-%d')"
...
run_benchmark() {
    ...
    setsid python -m sglang.launch_server --model-path "$model_path" --max-running-requests 128  --max-prefill-tokens 8192 --chunked-prefill-size 4096 > server.log 2>&1 &
    ...
    sleep 5
    local result
    if ! python3 -m sglang.bench_serving --backend sglang --dataset-name random --random-output-len 256 --random-input-len 8192 --num-prompts 5000 --random-range-ratio 1  --request-rate 16 > benchmark.log 2>&1; then
