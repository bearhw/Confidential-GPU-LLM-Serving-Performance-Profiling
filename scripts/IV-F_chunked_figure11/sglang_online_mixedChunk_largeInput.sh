...
OUTPUT_DIR="sglang_online_mixedChunk_nps4_largeInput8K_${MODE}_5000_$(date +'%Y-%m-%d')"
...
run_benchmark() {
    ...
    setsid python -m sglang.launch_server --model-path "$model_path" --max-running-requests 128  --max-prefill-tokens 8192 --enable-mixed-chunk --chunked-prefill-size 4096 > server.log 2>&1 &
