...
# Output CSV file
CSV_FILE="${OUTPUT_DIR}/sglang_offline_nps4_${MODE}_$(date +'%Y-%m-%d').csv"
...
run_benchmark() {
    ...
    setsid python -m sglang.launch_server --model-path "$model_path" --enable-torch-compile --max-running-requests "$max_num_seqs" --max-prefill-tokens 8192 > server.log 2>&1 &
