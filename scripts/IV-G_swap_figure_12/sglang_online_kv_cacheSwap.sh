...
OUTPUT_DIR="sglang_online_nps4_kv_cacheSwap_disableStorage_${MODE}_5000_$(date +'%Y-%m-%d')"
...
declare -a OUTPUT_LEN=(512 1024 2048 4096 8192)
log_error() {
...
run_benchmark() {
    local model_path=$1
    local output_len=$2
    echo "Running benchmark for $model_path with output_len $output_len..."
    cleanup_server
...
setsid python -m sglang.launch_server \
  ...
  > server.log 2>&1 &
    ...
    sleep 5
    local result
    if ! python3 -m sglang.bench_serving --backend sglang --dataset-name random --random-output-len "$output_len" --random-input-len 128 --num-prompts 5000 --request-rate 4 --random-range-ratio 1 > benchmark.log 2>&1; then
    ...
    echo "=== Benchmark Output for $model_path, output_len=$output_len ===" >> "$MASTER_LOG"
    cat benchmark.log >> "$MASTER_LOG"
    
    # Parse and store the latest result
    result=$(cat benchmark.log)
    # Parse KV eviction logs from server.log
    rm -f benchmark.log
    
    if ! echo "$result" | grep -q "Successful requests:"; then
        log_error "Benchmark output missing metrics for model $model_path with output_len $output_len"
...
echo "$model_path,$output_len,\"$ttfts_out\",\"$itls_out\",\"$e2e_latencies_out\",\"$tpots_out\"" >> "$RATIO_CSV_FILE" 
...
for model_path in "${MODEL_PATHS[@]}"; do
    for output_len in "${OUTPUT_LEN[@]}"; do
        ...
        if ! run_benchmark "$model_path" "$output_len"; then
            log_error "Benchmark run failed for model $model_path with output_len $output_len"
            continue
        fi
    done
done
# Compress logs into a zip file
zip benchmark_logs.zip "$MASTER_LOG" "$ERROR_LOG"
