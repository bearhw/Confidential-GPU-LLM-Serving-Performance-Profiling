...
# 최종 파일 이름 설정
OUTPUT_FILE="$OUTPUT_DIR/raw_bandwidth_${ENV_TAG}.txt"
...
if [ -f "./bandwidthTest" ]; then
    echo "Running benchmark (1KB to 64MB, 1MB step)..."
    ./bandwidthTest --mode=range --start=1024 --end=67109888 --increment=1048576 > "$OUTPUT_FILE"
    echo "------------------------------------------------"
