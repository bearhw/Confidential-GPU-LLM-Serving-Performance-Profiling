...
OUTPUT_DIR="sglang_online_req8_nps4_${MODE}_5000_$(date +'%Y-%m-%d')"
...
run_benchmark() {
    ...
    sleep 5
    local result
    if ! python3 -m sglang.bench_serving --backend sglang --dataset-name random --random-output-len 128 --random-input-len 128 --num-prompts 5000 --random-range-ratio 1  --request-rate 8 > benchmark.log 2>&1; then
